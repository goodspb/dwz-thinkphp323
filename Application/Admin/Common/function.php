...
{
    if (session('?nodeNameList')) {
        $name = session('nodeNameList');
        return $name [$id];
    }
    $Group = D("Node");
    $list = $Group->getField('id,name');
    $name = $list [$id];
    session('nodeNameList', $list);
    return $name;
}
...
function showStatus($status, $id, $callback = "")
{
    switch ($status) {
        case 0 :
            $info = '<a href="'.U(CONTROLLER_NAME.'/resume',array('id'=>$id ,'navTabId'=>MODULE_NAME)) . '" target="ajaxTodo" callback="' . $callback . '">恢复</a>';
            break;
        case 2 :
            $info = '<a href="'.U(CONTROLLER_NAME.'/pass',array('id'=>$id ,'navTabId'=>MODULE_NAME)) . '" target="ajaxTodo" callback="' . $callback . '">批准</a>';
            break;
        case 1 :
            $info = '<a href="'.U(CONTROLLER_NAME.'/forbid',array('id'=>$id ,'navTabId'=>MODULE_NAME)) . '" target="ajaxTodo" callback="' . $callback . '">禁用</a>';
            break;
        case -1 :
            $info = '<a href="'.U(CONTROLLER_NAME.'/recycle',array('id'=>$id ,'navTabId'=>MODULE_NAME)) . '" target="ajaxTodo" callback="' . $callback . '">还原</a>';
            break;
    }
    return $info;
}
...
function moduleAccess($moduleName)
{
    ...
    $list = M()->query("select m.id,m.name,m.group_id,m.title from node a, node m where a.id = m.pid and a.status=1 and m.status=1 and a.name='" . APP_NAME . "' and m.name='" . $moduleName . "'");

    $accessList = $_SESSION ['_ACCESS_LIST'];
    foreach ($list as $key => $module) {
        if (isset ($accessList [strtoupper(MODULE_NAME)] [strtoupper($module ['name'])])) {
            return true;
        }
    }
